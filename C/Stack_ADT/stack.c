...
static Node * findparent(const Stack *, const Item);    //그 노드를 가리키는 노드를 찾는다.
...
bool DeleteNode(Stack * pstack, Item si)
{
    ...
    Node * parent;                          //target node가 중간에 껴있을 때 사용
    //int temp;
    
    if (!(t_node = searching(pstack, si)))          //find target node using searching 
    {
...
/* ***** 지원 함수 ***** */
static Node * findparent(const Stack * pstack, const Item si)
{
    Node * parent;
